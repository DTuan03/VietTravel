...
public class ReviewPresenter implements ReviewContract.Presenter {
    ...
    private int userId; // Ensure userId is declared here

    private SharedPrefsHelper sharedPrefsHelper; // Declare sharedPrefsHelper here

    private static final Logger logger = Logger.getLogger(ReviewPresenter.class.getName());

    public ReviewPresenter(ReviewContract.View view, Activity activity) {
        this.view = view;
        sharedPrefsHelper = new SharedPrefsHelper(activity); // Initialize with activity context
        userId = sharedPrefsHelper.getInt("UserId"); // Retrieve userId as int from SharedPreferences, default value 0
    }
    ...
    public void submitFeedback(String userId, String idBookedTour, String comment, int rating, String date, String time, List<String> imageUris) {
        ...
        uploadImagesToFirebase(imageUris, new FirebaseUploadCallback() {
            ...
            public void onUploadSuccess(List<String> firebaseImageUrls) {
                new SaveFeedbackTask(idBookedTour, comment, rating, date, time, firebaseImageUrls).execute();
            }
    ...
    private class SaveFeedbackTask extends AsyncTask<Void, Void, Boolean> {
        ...
        SaveFeedbackTask(String idBookedTour, String comment, int rating, String date, String time, List<String> firebaseImageUrls) {
            this.idBookedTour = idBookedTour;
